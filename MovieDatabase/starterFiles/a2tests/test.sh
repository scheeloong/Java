...
username=ta        # Change this to your CDF account
database=a2test    # Change this your database name 'csc343h-cdf_account'
...
function test_part1
{
    ...
    if [ ! -f q6.sql ]; then
        ...
        psql -U $username -d $database -c "select * from imdb.movies;" >> .table.old 2>&1
        psql -U $username -d $1 -f $current/q6.sql >> temp
        psql -U $username -d $database -c "select * from imdb.movies;" >> .table.new 2>&1
